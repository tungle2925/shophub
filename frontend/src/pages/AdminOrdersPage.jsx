  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f5f5f5' }}>
      <div style={{ background: 'linear-gradient(135deg, #1a1a2e, #0f3460)', padding: '3rem 2rem', textAlign: 'center' }}>
        <h1 style={{ color: 'white', fontSize: '2.5rem', fontWeight: 800 }}>🛠️ Quản lý đơn hàng (Admin)</h1>
      </div>
      ...
      <div style={{ maxWidth: '1000px', margin: '2rem auto', padding: '0 2rem' }}>
        {orders.length === 0 ? (
          <div style={{ background: 'white', borderRadius: '16px', padding: '3rem', textAlign: 'center', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
            <p style={{ color: '#888' }}>Chưa có đơn hàng nào trong hệ thống.</p>
          </div>
        ) : (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#f8f9fa', borderBottom: '1px solid #eee' }}>
                  ...
                  <th style={{ padding: '1rem', textAlign: 'center', color: '#1a1a2e' }}>Quản lý</th>
                </tr>
              </thead>
              <tbody>
                {orders.map((o) => (
                  <tr key={o.id} style={{ borderBottom: '1px solid #eee' }}>
                    ...
                    <td style={{ padding: '1rem', textAlign: 'center' }}>
                      <Link to={`/admin/orders/${o.id}`} style={{ color: '#e94560', fontWeight: 600 }}>Quản lý</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
